...
void CMarkupSTL::operator=( const CMarkupSTL& markup )
{
	...
	if ( m_aPos.nSegs )
	{
		...
		for ( int nSeg=0; nSeg < m_aPos.nSegs; ++nSeg )
		{
			...
			memcpy( (void*)(m_aPos.pSegs[nSeg]), markup.m_aPos.pSegs[nSeg], nSegSize*sizeof(ElemPos) );
		}
	}
...
bool CMarkupSTL::ReadTextFile( const char* szFileName, std::string& strDoc, std::string* pstrError, int* pnFlags )
{
...
bool CMarkupSTL::WriteTextFile( const char* szFileName, const std::string& strDoc, std::string* pstrError, int* pnFlags )
{
...
std::string CMarkupSTL::EscapeText( const char* szText, int nFlags )
{
	...
	while ( cSource )
	{
		if ( (pFound=strchr(pFind,cSource)) != NULL )
		{
			...
			if ( (nFlags&MNF_WITHREFS) && *pFound == '&' )
			{
				...
				++pCheckEntity;
				unsigned char c = (unsigned char)*pCheckEntity;
				if ( (c>='A'&&c<='Z') || (c>='a'&&c<='z')
						|| c=='#' || c=='_' || c==':' || c>0x7f )
				{
					while ( true )
...
std::string CMarkupSTL::UnescapeText( const char* szText, int nTextLength /*=-1*/ )
{
...
//[CMARKUPDEV
bool CMarkupSTL::SetDoc( const std::string& strDoc )
{
	m_strDoc = strDoc;
	m_strError.erase();
	return x_ParseDoc();
...
std::string CMarkupSTL::GetDeclaredEncoding( const char* szDoc )
{
	...
	if ( pStart && pStart[1] == '?' )
	{
		...
		if ( pEnd )
		{
			std::string strXMLDecl( pStart, (int)(pEnd-pStart)+2 );
...
//[CMARKUPDEV
std::string CMarkupSTL::FindGetData( const char* szName )
{
	if ( FindElem(szName) )
...
std::string CMarkupSTL::EncodeBase64( const unsigned char* pBuffer, int nBufferLen )
{
...
int CMarkupSTL::DecodeBase64( const std::string& strBase64, unsigned char* pBuffer, int nBufferLen )
{
...
std::string CMarkupSTL::UTF8ToA( const char* pszUTF8 )
{
	// Converts from UTF-8 to locale ANSI charset (via wide std::string)
	...
	// When larger than 255 wide chars an intermediate wide std::string buffer is allocated
	// And when result std::string is larger than 255 another intermediate buffer is allocated
...
std::string CMarkupSTL::AToUTF8( const char* pszANSI )
{
...
bool CMarkupSTL::x_ParseDoc()
{
	...
	std::string strResult = m_strError;
...
std::string CMarkupSTL::x_GetToken( const CMarkupSTL::TokenPos& token )
{
...
std::string CMarkupSTL::x_GetPath( int iPos ) const
{
	std::string strPath;
	while ( iPos )
	{
...
std::string CMarkupSTL::x_EncodeCDATASection( const char* szData )
{
	...
	while ( pszEnd )
	{
		strData += std::string( pszNextStart, (int)(pszEnd - pszNextStart) );
		strData += "]]]]><![CDATA[>";
...
std::string CMarkupSTL::x_GetElemContent( int iPos ) const
{
...
int CMarkupSTL::x_InsertNew( int iPosParent, int& iPosRel, CMarkupSTL::NodePos& node )
{
	...
	if ( bEmptyParentTag )
	{
		std::string strTagName = x_GetTagName( iPosParent );
		std::string strFormat;
		if ( node.nFlags & MNF_WITHNOLINES )
			strFormat = ">";
		else
...
std::string CMarkupSTL::x_GetSubDoc( int iPos ) const
{
	if ( iPos )
	{
...
void CMarkupSTL::x_RemoveNode( int iPosParent, int& iPos, int& nNodeType, int& nNodeOffset, int& nNodeLength )
{
	...
	// Remove node from document
 	x_DocChange( nNodeOffset, nNodeLength, std::string() );
