...
#ifdef _DEBUG
#define MARKUPSTL__DS(i) (i?&(m_strDoc.c_str())[m_aPos[i].nStart]:0)
#define MARKUPSTL_SETDEBUGSTATE m_pMainDS=MARKUPSTL__DS(m_iPos); m_pChildDS=MARKUPSTL__DS(m_iPosChild)
#else
#define MARKUPSTL_SETDEBUGSTATE
#endif
...
public:
    ...
    ~CMarkupSTL() { m_strDoc = ""; m_strError = ""; };
	...
	std::string GetParentElemPath() const { return x_GetPath(m_iPosParent); };
	...
	static bool WriteTextFile( const char* szFileName, const std::string& strDoc, std::string* pstrError=NULL, int* pnFlags=NULL );
...
protected:
	...
	struct TokenPos
	{
		...
		void Clear() { nL=0; nR=-1; nNext=0; nPreSpaceStart=0; nPreSpaceLength=0; };
